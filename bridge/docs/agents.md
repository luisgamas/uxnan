...
is closed (the one-shot CLIs hang on an open stdin pipe). The two server-based
adapters are the exception: **Codex** speaks JSON-RPC over a long-lived
`codex app-server` stdio, and **OpenCode** speaks HTTP + SSE to a long-lived
`opencode serve` process (the prompt travels in the request body, never argv).
...
and Gemini (`BeforeTool` hook); only **pi** has no headless pre-tool channel yet
(it runs autonomously — see [`../FOR-DEV.md`](../FOR-DEV.md)).
...
once per turn) or a **long-lived server** (`codex-adapter.ts` over stdio JSON-RPC,
`opencode-adapter.ts` over `opencode serve` HTTP/SSE, when the CLI exposes a pre-tool
approval channel). Adjust the args/request builder + event parser, register it in
