 ...
 *
 * FOR-DEV / limitations (see bridge/FOR-DEV.md):
 *  - `applyCheckpoint` restores file CONTENTS from the snapshot (worktree only,
 *    via `git restore`); it does not delete files created after the checkpoint.
 *  - checkpoint commits use a fixed internal identity (never pushed).
 */
...
export class CheckpointService {
  ...
  async capture(cwd: string, options: CaptureOptions): Promise<Checkpoint> {
    ...
    records.push(record);
    await this.#state.writeJson(DAEMON_FILES.checkpoints, records);
