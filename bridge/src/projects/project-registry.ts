...
import { JsonRpcErrorCode, RpcError, type Project } from '@uxnan/shared';
...
export class ProjectRegistry {
  ...
  resolve(cwd: string): Project {
    ...
    return toProject(match ?? target);
  }
}

function toProject(cwd: string): Project {
  return { id: projectIdFor(cwd), name: basename(cwd) || cwd, cwd };
}
