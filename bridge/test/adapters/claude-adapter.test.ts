...
// --- interactive-approval path (stdin kept open; control_request/response) ---
...
function fakeInteractiveSpawner(): {
  spawnFn: (command: string, args: string[], cwd: string) => SpawnedProcess;
  last(): FakeInteractive;
} {
  let handle: FakeInteractive | undefined;
  const spawnFn = (_command: string, args: string[]): SpawnedProcess => {
    const stdout = new PassThrough();
    const emitter = new EventEmitter();
    const chunks: string[] = [];
  ...
  return { spawnFn, last: () => handle! };
}

const tick = (): Promise<void> => new Promise((r) => setTimeout(r, 20));

test('interactive approvals: control_request → approval block; decision → control_response; then completes', async () => {
  const { spawnFn, last } = fakeInteractiveSpawner();
  const adapter = new ClaudeCodeAdapter({ interactiveApprovals: true, interactiveSpawnFn: spawnFn });
  const events: AgentStreamEvent[] = [];
  let resolveDone!: () => void;
  const done = new Promise<void>((r) => (resolveDone = r));
  adapter.onEvent((e) => {
    events.push(e);
    if (e.type === 'turn_completed') resolveDone();
  });

  await adapter.sendTurn({ threadId: 't', turnId: 'u', text: 'clean it up' });
  const proc = last();
  // The interactive flags + the prompt fed as a stream-json user message.
  assert.ok(proc.args.includes('--input-format'));
  assert.ok(proc.stdin().some((c) => c.includes('"type":"user"') && c.includes('clean it up')));
  ...
  assert.ok(block, 'expected an approval block');
  const content = (block!.data as { content: { type: string; approvalId: string } }).content;
  assert.equal(content.type, 'approval');

  // The user approves → a control_response with behavior:allow is written to stdin.
  await adapter.respondApproval!('t', content.approvalId, 'approve');
  assert.ok(proc.stdin().some((c) => c.includes('"behavior":"allow"') && c.includes('r1')));

  // The CLI then finishes the turn.
  proc.feed('{"type":"result","subtype":"success","is_error":false,"result":"cleaned","session_id":"s"}');
  await done;
  const completed = events.find((e) => e.type === 'turn_completed');
  assert.equal((completed!.data as { text: string }).text, 'cleaned');
});

test('interactive approvals: reject writes a deny control_response', async () => {
  const { spawnFn, last } = fakeInteractiveSpawner();
  const adapter = new ClaudeCodeAdapter({ interactiveApprovals: true, interactiveSpawnFn: spawnFn });
  const events: AgentStreamEvent[] = [];
  ...
  proc.feed(
    JSON.stringify({
      type: 'control_request',
      request_id: 'r9',
      request: { subtype: 'can_use_tool', tool_name: 'Write', input: { file_path: '/etc/hosts' } },
    }),
  );
  await tick();
  const content = (events.find((e) => e.type === 'block')!.data as { content: { approvalId: string } })
    .content;
  await adapter.respondApproval!('t', content.approvalId, 'reject');
  assert.ok(proc.stdin().some((c) => c.includes('"behavior":"deny"')));
});
