...
- **Hand-kept model tables come in PAIRS — a new model must be added to BOTH.**
  ...
  | Gemini | `bridge/src/adapters/gemini-adapter.ts` → `GEMINI_MODELS` | `uxnandesktop/src-tauri/src/agentcli.rs` → `GEMINI_MODELS` — the desktop no longer *offers* Gemini for AI commit / PR body (discontinued upstream), so this half only serves a config that already named it |
