...
- `uxnandesktop/` — **in progress**: Tauri 2 + Rust + Svelte 5 ADE. **Phases 0–3 complete** — three-panel resizable shell with atomic JSON persistence, PTY terminals (`portable-pty`, xterm WebGL + DOM fallback) with tabs and nested splits that never remount on split, git worktrees with per-worktree terminal workspaces + in-app directory picker, git status/diff/stage/commit/push/pull with a CodeMirror 6 diff viewer and a 3s Tokio status watcher. **Agent monitoring (Phase 4) underway** — activity inference (output→working, quiet≥3s→idle, exit→done), native idle notifications, per-agent sidebar rows, process-tree detection. Cross-cutting done: full EN/ES i18n, design tokens, agent registry/catalog with install detection, shell-aware manual + opt-in auto agent launch. Pending (tracked in `uxnandesktop/FOR-DEV.md`): precise hook-based agent states (rest of Phase 4), polish (Phase 5: hunk staging, side-by-side diffs, virtual scroll, persistence hardening), and the embedded bridge for mobile pairing (Phase 6).
