...
- `bridge/` — **implemented**: E2EE transport (relay + LAN), **OpenCode, Claude Code and Codex wired as real agents** (each spawns its official local CLI over stdio — no provider API/SDK/keys), per-thread agent/model/project selection, **account-aware structured model discovery** (`AgentModel[]`: Codex via `codex app-server` `model/list`, Claude aliases "(latest)" + config-pinned versions + resolved-version `model_resolved` event), **per-turn token usage** on `turn/completed`, **directory browsing** (`workspace/browseDirs`), Git + workspace + checkpoints, conversation engine, push (gated), resilient relay reconnection. The remaining agent (Gemini) follows the recipe in `bridge/FOR-DEV.md`.
