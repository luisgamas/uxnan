...
- **Hand-kept model tables come in PAIRS — a new model must be added to BOTH.**
  ...
  **Gemini CLI is deprecated: do not spend work on it.** It is discontinued
  ...
  agent and which discovers its own models). Both apps retire it — the
  desktop drops it from the AI-commit/PR-body pickers and no longer auto-installs
  its reporter; mobile has no Gemini enum, visual, provider card or conversation
  surface — and its tables are **frozen**:
  don't add models to them, don't chase upstream changes, and don't build new
  features against it. The bridge descriptor is `deprecated:true` and
  unavailable; new threads/turns, models and commands are rejected. It will be
  removed from the project entirely in a later pass; until then, treat every
  Gemini path as non-runnable, read-only legacy. Same for anything else it
  touches (adapter, hook reporter, catalog entry): don't extend it.
