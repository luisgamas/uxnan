...
- `uxnandesktop/` — **in progress**: Tauri 2 + Rust + Svelte 5 ADE. **Phases 0–4 complete** — three-panel resizable shell with atomic JSON persistence, PTY terminals (`portable-pty`, xterm WebGL + DOM fallback) with tabs and nested splits that never remount on split, git worktrees with per-worktree terminal workspaces + in-app directory picker, git status/diff/stage/commit/push/pull with a CodeMirror 6 diff viewer and a 3s Tokio status watcher. **Agent monitoring (Phase 4) done** — activity inference (output→working, quiet≥3s→idle, exit→done) + native idle notifications + per-agent sidebar rows + process-tree detection, plus the **Layer 1 local HTTP hook server** (`axum`: agents POST precise `working/blocked/waiting/done`, token-guarded, env-injected `UXNAN_HOOK_URL`/`_TOKEN`/`_AGENT_ID`, persistent cache TTL 7d), **Layer 2** terminal-title (OSC) inference, colored status dots, unread/done badges (sidebar + dock count), custom agent logos, and per-worktree agent override. Cross-cutting done: full EN/ES i18n, design tokens, agent registry/catalog with install detection, shell-aware manual + opt-in auto agent launch. Pending (tracked in `uxnandesktop/FOR-DEV.md`): Phase-4 follow-ups (ready-made per-agent hook configs, multi-agent orchestration), polish (Phase 5: hunk staging, side-by-side diffs, virtual scroll, persistence hardening), and the embedded bridge for mobile pairing (Phase 6).
