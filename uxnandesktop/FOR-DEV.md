...
`docs/chat.md`). 1,002 Rust tests (925 unit in the app crate + 18 in `uxnan-control-protocol` + 14 in `uxnan-cli` + 45
...
GitHub tests, 1 real-scheduler probe) + 1,512 passing frontend Vitest tests across two
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 1,002 Rust + 1,512 passing Vitest tests (both
