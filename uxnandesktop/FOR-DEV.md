...
always wins). 586 passing Rust tests (557 unit + 29
integration; +7 ignored supervised live GitHub tests + 1 ignored real-scheduler
probe) + 1,061 passing frontend Vitest tests (+7 skipped) across two
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 586 Rust + 1,061 passing Vitest tests (both
