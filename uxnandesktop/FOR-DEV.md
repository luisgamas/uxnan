...
always wins). 589 Rust tests (560 unit + 29
integration; +7 ignored supervised live GitHub tests + 1 ignored real-scheduler
probe) + 1,061 passing frontend Vitest tests across two
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 589 Rust + 1,061 passing Vitest tests (both
