...
always wins). 906 Rust tests (837 unit in the app crate + 18 in `uxnan-control-protocol` + 14 in `uxnan-cli` + 37
...
GitHub tests, 1 real-scheduler probe) + 1,304 passing frontend Vitest tests across two
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 906 Rust + 1,304 passing Vitest tests (both
