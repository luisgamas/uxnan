...
always wins). 808 Rust tests (771 unit + 37
integration), of which 49 are ignored probes that need something real to talk to
(40 live SSH probes — 29 against a real `sshd` and 11 against a **Linux host in a
container**, `npm run test:ssh:linux` — one pwsh preflight, 7 supervised live
GitHub tests, 1 real-scheduler probe) + 1,228 passing frontend Vitest tests across two
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 808 Rust + 1,228 passing Vitest tests (both
