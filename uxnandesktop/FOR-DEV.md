...
always wins). 947 Rust tests (870 unit in the app crate + 18 in `uxnan-control-protocol` + 14 in `uxnan-cli` + 45
...
**Done (groups `read`, `ui`, `create` and `converse`, and the surface itself):** the workspace crate
...
`run start` / `automation ls|run`, `--prompt-file`, `--idempotency-key`. The
`converse` group: `agent/send` (paste-and-submit through the backpressure
...
*busy* with `live`/`cap` before anything is created); **a worker is
unattended by design**: `worker/start` launches the CLI in its reviewed
automatic mode unless the caller says `unattended: false` (`--attended`) or
the agent's Settings switch (*Automatic mode when launched by an agent*,
`AgentProfile.workersUnattended`) is off; the table
(`src/lib/agentUnattended.ts`) covers every catalog CLI with a reviewed or
edits-only tier (`claude`, `codex`, `qwen`, `ante`, `kimi`, `devin`, `goose`
by env / `agy`, `grok`, `command-code`, `vibe`, `omp`, `autohand`), receipt
`applied` / `partial` / `configured` / `unsupported`; `terminal/create` and
`worktree/create` with an agent stay opt-in — verified live (before the
default) with an unattended Codex worker that ran the MCP tools and reported
with no prompt, once its hook trust was seeded under the right key
(`codex_trust`, group index read back from `hooks.json`) and the multi-line
preamble submitted (400 ms post-paste Enter). A coordinator collects the
workers it started with `terminal/close` (a tab the surface opened, once its
agent is done, or any exited one; a person's live tab and a working agent are
...
### Still pending
- [ ] **`automation/propose` — let an agent draft an automation for a person to
      save.** Creating, editing, enabling or scheduling an automation is not
      exposed and should not be: one that creates itself and schedules itself
      **outlives the session that made it**. The useful half is a `ui` entry
      that opens the automations editor with a definition filled in, so the
      person presses save — the same shape as `file/diff` (the agent shows, the
      person decides). Left out of the hosts/automations round (2026-09-23)
      because it is UI, and UI is reviewed on screen before it is committed.
      Sites: `crates/control-protocol/src/catalog.rs` (the entry),
      `control/services/automation.rs`, `src/lib/control/bridge.ts` (the window
      handler) and the automations editor.
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 947 Rust + 1,327 passing Vitest tests (both
