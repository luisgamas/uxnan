...
with" external editors/IDEs**). 353 Rust backend tests + 275 frontend Vitest unit tests (pure logic); **no Svelte component or E2E tests yet**. macOS now ships an
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 353 Rust + 275 Vitest tests.
