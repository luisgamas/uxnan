...
always wins). 913 Rust tests (838 unit in the app crate + 18 in `uxnan-control-protocol` + 14 in `uxnan-cli` + 43
...
- ✅ **Verify** — `.github/workflows/ci-desktop.yml` runs svelte-check + `npm test`
  ...
  keeps the default `{ubuntu, windows}`. 913 Rust + 1,305 passing Vitest tests (both
