...
// FOR-DEV: no caller yet — the Tauri command surface for Settings -> Hosts is
// what drives this. Landing the data logic first keeps the part that can lose a
// user's projects reviewable on its own. Remove with the commands.
#![allow(dead_code)]
...
/// Find an already-registered host for this draft, if any.
pub fn find_existing<'a>(hosts: &'a [SshHost], draft: &HostDraft) -> Option<&'a SshHost> {
    let wanted = draft.machine_key();
    hosts.iter().find(|h| h.machine_key().same(&wanted))
}
