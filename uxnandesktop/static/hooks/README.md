...
| `uxnan-opencode-status-plugin.js` | OpenCode, MiMo Code, Kilo Code | An in-process ES-module plugin (installed into each CLI's own plugin dir). Re-labels the bus events to the hook server's vocabulary and POSTs directly. One default-exported `{ id, setup, server }` descriptor speaks both plugin APIs — `setup` streams OpenCode 2's events, `server` is the V1 factory OpenCode 1, MiMo and Kilo call — so all three CLIs and both OpenCode majors run this one source; the installer rewrites only the agent kind it declares. It stays silent inside OpenCode 2's shared background service, whose environment is another tab's. |
