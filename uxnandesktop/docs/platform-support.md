...
| `credstore.rs` (OS credential store, read-only) | macOS: Security.framework generic-password read with user interaction disabled on the polling path, enabled only for the user-initiated grant (`usage_grant_access`); Windows / Linux: `Unsupported` stub | Error-code mapping + a quiet read that must return without prompting and leave interaction re-enabled (runs on the `macos-14` leg); the Claude Keychain item derivation is unit-tested in `usage.rs` on every runner | The lived grant flow (dialog → *Always Allow* → silent polls and restart, revocation) was walked once on real Apple Silicon by the maintainer (2026-09-19, dev build, PR #240); it rides the macOS feature smoke from here, like the rest of the Mac-only branches |
...
| `keyboard/`, `keyboard.rs`, `menu.rs`, `Terminal.svelte`, `WindowControls.svelte`, dialogs | `isMac` modifier mapping (⌘ vs Ctrl), per-platform default chords, chord rendering, the macOS menu bar, global shortcuts inside a browser page (menu bar / WebView2 accelerator keys / GTK key presses), WebView2's browser keys off in release, native macOS controls vs custom Windows/Linux controls | Platform selection/config is contract-tested, traffic-light placement now against measured values; input behavior remains a visible check in each platform's smoke checklist |
