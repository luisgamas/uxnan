...
| `credstore.rs` (OS credential store, read-only) | macOS: Security.framework generic-password read with user interaction disabled on the polling path, enabled only for the user-initiated grant (`usage_grant_access`); Windows / Linux: `Unsupported` stub | Error-code mapping + a quiet read that must return without prompting and leave interaction re-enabled (runs on the `macos-14` leg); the Claude Keychain item derivation is unit-tested in `usage.rs` on every runner | The lived grant flow (dialog → *Always Allow* → silent polls, revocation, re-grant after a rebuild) needs real Apple hardware → `FOR-DEV.md` → *Providers* + mac checklist |
