...
consumed — quota/rate windows (percent **used** + reset countdown), your plan and
account, and any credit balance — for the providers you activate. Only activated
providers are ever read, so an idle feature costs nothing.
...
|---|---|---|
| **Codex** | `~/.codex/auth.json` | monthly/weekly windows, plan, credit, email |
...
3. Each activated provider gets a **tab** showing its live data:
   - **Quota windows** — a bar per window; the number is the **percent consumed**
     of that limit, with the reset countdown.
   - **Account** — "Authenticated as …"; the email/login is blurred until you
     click it (click again to hide).
   - **Credit** — balance when the provider exposes one (Codex/Claude).
   - **Refresh interval** — per-provider override of the global interval.
   - **Status bar** — which of this provider's windows / plan / credit surface in
     the bottom status-bar popover (the primary % window is on by default).
