...
[`github-sandbox-runbook.md`](github-sandbox-runbook.md)). **682 backend tests**
in total, 655 of which run everywhere; the other 27 are ignored probes that need
something real to talk to (19 live SSH probes against a real `sshd`, the 7
supervised live GitHub tests, the real-scheduler probe).

The 626 passing unit tests cover the Serde model shape, persistence round-trip / atomicity /
...
[`platform-support.md`](platform-support.md)). **1,113 passing tests** across both projects,
