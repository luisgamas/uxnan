...
install-policy decision), `diffParse.ts` (unified-diff parsing), `theme.ts`
(batch theme-import normalization) and `quickCommands.ts` (quick-command token
substitution + scope filters) — 127 tests in `src/lib/*.test.ts`, config in
