...
[`github-sandbox-runbook.md`](github-sandbox-runbook.md)). **542 backend tests**
in total (plus the 7 ignored live tests and the ignored real-scheduler probe).

The 513 passing unit tests cover the Serde model shape, persistence round-trip / atomicity /
...
ships in every build and is never shown). **1,061 passing tests** across both
