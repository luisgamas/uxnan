...
[`github-sandbox-runbook.md`](github-sandbox-runbook.md)). **808 backend tests**
in total, 759 of which run everywhere; the other 49 are ignored probes that need
something real to talk to (40 live SSH probes — 29 against a real `sshd`, one of
...
The 729 passing unit tests (771 with the ignored probes) cover the Serde model shape, persistence round-trip / atomicity /
...
ships in every build and is never shown). **1,228 passing tests** across both
