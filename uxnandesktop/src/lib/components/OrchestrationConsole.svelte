<script lang="ts">
  // Multi-agent orchestration console (spec 02d §3). Lists every running agent
  // grouped by type, lets you mark one as the coordinator (task-graph root), and
  // routes a message to all agents, to one type (fan-out), or to the
  // coordinator's workers. Delivery is backpressured: each agent gets the next
  // queued message only once it reports free again.
...
<Dialog.Root bind:open={app.orchestrationOpen}>
  <Dialog.Content class="sm:max-w-[640px]">
    <Dialog.Header>
      ...
      <Dialog.Description>{i18n.t("orchestration.desc")}</Dialog.Description>
    </Dialog.Header>
