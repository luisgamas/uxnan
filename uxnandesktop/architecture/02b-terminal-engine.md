...
- **Legibilidad garantizada**: los valores por defecto de la terminal (texto, cursor y paleta ANSI) se derivan de la **luminancia del fondo ya resuelto**, no de la base del tema de la app — así un preset de terminal oscuro bajo un tema claro (o al revés) nunca hereda los colores de la base contraria y deja campos sin definir del mismo color que el fondo. Encima, xterm aplica un **suelo de contraste** (`minimumContrastRatio`) que levanta cualquier glifo demasiado cercano al fondo de su celda: **4.5** (WCAG AA) sobre fondo claro y **3.0** (AA texto grande) sobre fondo oscuro. Es la única defensa frente a colores que el ADE no controla — un TUI pintando gris de 24 bits sobre gris, o `ANSI black` sobre un fondo casi negro — y sobre fondo oscuro se mantiene en 3.0 a propósito, para no lavar los colores saturados de las paletas oscuras. Detalle en `../docs/theming.md` → *Terminal legibility*.
...
- **Protocolo de teclado moderno (Kitty / "CSI u")**: xterm.js no lo trae de fábrica, así que el ADE implementa la mitad del terminal en `src/lib/terminal/keyboardProtocol.ts`. Está **inactivo hasta que una app lo negocia** (vía `CSI ? u` / `CSI > u` / `CSI < u` / `CSI = u`, registradas con `registerCsiHandler`), de modo que el manejo de teclas existente no cambia. Activo, codifica las teclas como `CSI <code> ; <mods> [: <event>] u` y desambigua `Ctrl+I`/Tab, `Esc` y combinaciones `Ctrl`/`Alt`+letra; las teclas funcionales/de navegación caen a la codificación legacy de xterm (pendientes en `FOR-DEV.md`). En macOS, Option **compone** caracteres (`@` es ⌥Q en la distribución latinoamericana, ⌥2 en la española): una tecla así se envía como su texto, nunca como `Alt`+tecla; Option con ⌘ o ⌃, y Option+Enter, conservan su modificador. El scroll en pantalla alterna (rueda → flechas) lo provee xterm.js de forma nativa.
...
3. **Tab se oculta** → Backend Rust continúa leyendo PTY stdout y emitiendo `pty:output:{id}` → el xterm (siempre montado, oculto con `display:none`) sigue consumiendo el stream hacia su buffer, así que no se pierde output ni scrollback; el pane oculto libera su contexto WebGL y repinta al revelarse.
   ...
   - **Seleccionar / menú contextual del tab** → un **clic izquierdo** activa el tab (`terminals.setActiveTab`; un arrastre más allá del umbral lo reordena en su lugar). Un **clic derecho** solo **abre el menú contextual** del tab (renombrar · split · cerrar · cerrar todo) **sin cambiar el tab activo** — sus acciones apuntan al tab clicado por `id`, así que clic-derecho + Cerrar sobre un tab de fondo no salta primero a él. Seleccionar es exclusivo del clic izquierdo, uniforme en Windows/Linux/macOS. El mismo principio rige las pestañas `bits-ui` compartidas (panel derecho Files/Changes/History/GitHub, Settings, etc.): el wrapper `ui/tabs` usa `activationMode="manual"` por defecto, así que una pestaña se activa solo con clic/Enter/Espacio y **nunca por foco** — en Windows un clic derecho da foco al botón, y el modo "automatic" de bits-ui cambiaba de pestaña con ese foco.
