...
| **S/Follow-up** | E2E (WebdriverIO + tauri-driver) + component tests | ✅ Done | Current required suite: `cargo test` 529 passing (501 unit + 28 integration) plus 8 ignored supervised/host probes; `npm test` 907 passing; `npm run check` 0 errors / 0 warnings; `cargo clippy`; `cargo fmt`. The real packaged-app E2E suite runs manually/nightly on Windows; physical macOS/Linux validation remains tracked separately in the platform matrix. |
