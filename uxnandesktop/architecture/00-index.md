...
| **6** | Bridge integration (mobile pairing) | 🟡 Parcial | **Hecho:** el desktop como **cliente** de un bridge instalado por el canal de control local (`02e` §3.5; modos off/attach/managed) y **pestañas de chat** que conducen los hilos del bridge junto al telefono (plan 030 etapas B–C). **Pendiente:** sidecar empaquetado + QR pairing y gestion de dispositivos desde el desktop; espejo/relevo de sesiones lanzadas en terminal. **Opcional para standalone**; el bridge standalone es la referencia del contrato |
...
| **S/Follow-up** | AI-provider usage | ✅ Done | Settings → Providers reads the activated Codex, Claude, Copilot, and Grok CLI credentials natively to show quota windows, resets, plan/account, and credit without cookies or pasted keys. Polling starts at boot, catches stale snapshots on focus, honors each provider's interval, and keeps Codex `used_percent` values as percentage points across resets. On macOS, Claude Code's token is read from the login Keychain behind an explicit, OS-mediated grant (`credstore.rs`: polls never prompt; `accessRequired` + *Grant access*). **Antigravity** remains deferred on its unverified keyring item names; findings and the unblock path live in `FOR-DEV.md` → *Providers*. |
...
| **S/Follow-up** | E2E (WebdriverIO + tauri-driver) + component tests | ✅ Done | Current required suite: `cargo test` 1,001 tests — 952 passing (924 unit in the app crate + 18 in `uxnan-control-protocol` + 14 in `uxnan-cli` + 45 integration) plus 49 ignored supervised/host probes (including a Linux SSH host in Docker); `npm test` 1,512 passing; `npm run check` 0 errors / 0 warnings; `cargo clippy`; `cargo fmt`. The real packaged-app E2E suite runs manually/nightly on Windows; physical macOS/Linux validation remains tracked separately in the platform matrix. |
