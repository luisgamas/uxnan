...
| **S/Follow-up** | E2E (WebdriverIO + tauri-driver) + component tests | ✅ Done | Current required suite: `cargo test` 1,004 tests — 955 passing (927 unit in the app crate + 18 in `uxnan-control-protocol` + 14 in `uxnan-cli` + 45 integration) plus 49 ignored supervised/host probes (including a Linux SSH host in Docker); `npm test` 1,538 passing; `npm run check` 0 errors / 0 warnings; `cargo clippy`; `cargo fmt`. The real packaged-app E2E suite runs manually/nightly on Windows; physical macOS/Linux validation remains tracked separately in the platform matrix. |
