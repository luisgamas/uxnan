...
| **S/Follow-up** | AI-provider usage | ✅ Done | Settings → Providers reads the activated Codex, Claude, Copilot, and Grok CLI credentials natively to show quota windows, resets, plan/account, and credit without cookies or pasted keys. Polling starts at boot, catches stale snapshots on focus, honors each provider's interval, and keeps Codex `used_percent` values as percentage points across resets. **Gemini CLI** remains readable but is deprecated and hidden from Add provider. **Antigravity** remains deferred because `agy` stores its token in the OS keyring; findings and the unblock path live in `FOR-DEV.md` → *Providers*. |
...
| **S/Follow-up** | E2E (WebdriverIO + tauri-driver) + component tests | ✅ Done | Current required suite: `cargo test` 477 passing (449 unit + 28 integration) plus 8 ignored supervised/host probes; `npm test` 719 passing + 7 platform-skipped; `npm run check` 0 errors / 0 warnings; `cargo clippy`; `cargo fmt`. The real packaged-app E2E suite runs manually/nightly on Windows; physical macOS/Linux validation remains tracked separately in the platform matrix. |
