...
| **S/Follow-up** | Mascotas (companero animado) | ✅ Hecho | Opt-in (apagado por defecto): refleja el estado preciso de los agentes (`working`→`running`, `waiting`→`waiting`, `done`→`review`, `blocked`→`failed`, si nadie reporta `idle`; stale ignorado), como **una mascota global** (gana el mas urgente) **o una por agente** (colonia); vive como capa dentro de la ventana de uxnan (una ventana de escritorio propia queda pendiente); clic revela la terminal de su agente. Formato en disco **compatible con Codex**, asi que los paquetes de la comunidad cargan sin cambios; importar desde `~/.codex/pets` o cualquier carpeta es una **copia validante**. uxnan incluye **solo su propia mascota** (arte final = item de `FOR-HUMAN`). Interruptor en el menu de perfil + Settings → Pets (`02d` §1.7, `docs/pets.md`). Generacion con IA ("hatch") queda como follow-up |
| **S/Follow-up** | E2E tests (Playwright / WebdriverIO + tauri-driver) + component tests | ⏳ Pendiente | Ya en verde: `cargo test` 274 passing + `npm test` 249 (Vitest, logica pura) + `npm run check` 0/0 + `cargo clippy` + `cargo fmt`. Faltan E2E y tests de componentes Svelte |
