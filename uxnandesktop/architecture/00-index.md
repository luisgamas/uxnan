...
| **S/Follow-up** | Consumo de proveedores AI | ✅ Hecho | Settings → Providers lee de forma nativa y opt-in las credenciales propias de Codex, Claude, Copilot y Grok para mostrar ventanas de cuota, reinicios, plan/cuenta y crédito sin cookies ni claves pegadas por el usuario. **Gemini CLI** sigue leyéndose pero ya no se ofrece al agregar un proveedor (descontinuada aguas arriba; `deprecated` en `src/lib/usageCatalog.ts`, se re-habilita quitando la bandera). **Antigravity** no es proveedor todavía: su cuota vive tras la misma API de Code Assist, pero `agy` guarda su token en el llavero del SO y no en disco — hallazgos y desbloqueo en `FOR-DEV.md` → *Providers*. |
...
| **S/Follow-up** | E2E tests (Playwright / WebdriverIO + tauri-driver) + component tests | ⏳ Pendiente | Ya en verde: `cargo test` 368 passing + `npm test` 362 (Vitest, logica pura) + `npm run check` 0/0 + `cargo clippy` + `cargo fmt`. Faltan E2E y tests de componentes Svelte |
