...
| **6** | Bridge integration (mobile pairing) | 🟡 Parcial | **Hecho:** el desktop como **cliente** de un bridge instalado por el canal de control local (`02e` §3.5; modos off/attach/managed) y **pestañas de chat** que conducen los hilos del bridge junto al telefono (`02a` §5.8.16). **Pendiente:** sidecar empaquetado + QR pairing y gestion de dispositivos desde el desktop; espejo/relevo de sesiones lanzadas en terminal. **Opcional para standalone**; el bridge standalone es la referencia del contrato |
...
| **S/Follow-up** | E2E (WebdriverIO + tauri-driver) + component tests | ✅ Done | Current required suite: `cargo test` 1,004 tests — 955 passing (927 unit in the app crate + 18 in `uxnan-control-protocol` + 14 in `uxnan-cli` + 45 integration) plus 49 ignored supervised/host probes (including a Linux SSH host in Docker); `npm test` 1,536 passing; `npm run check` 0 errors / 0 warnings; `cargo clippy`; `cargo fmt`. The real packaged-app E2E suite runs manually/nightly on Windows; physical macOS/Linux validation remains tracked separately in the platform matrix. |
