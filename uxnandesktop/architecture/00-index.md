...
| **S/Follow-up** | E2E tests (Playwright / WebdriverIO + tauri-driver) + component tests | ⏳ Pendiente | Ya en verde: `cargo test` 337 passing + `npm test` 258 (Vitest, logica pura) + `npm run check` 0/0 + `cargo clippy` + `cargo fmt`. Faltan E2E y tests de componentes Svelte |
