...
| **Custom themes** (temas personalizables) | ✅ Hecho | El usuario diseña temas Material 3 completos (light + dark, todos los roles M3) en una **librería multi-tema** y selecciona cuál aplica (o ninguno, volviendo a System/Light/Dark) desde la misma pantalla de Personalización. `buildUxnanTheme(themeSource: …)` con `ThemeSource.brand` (paleta hand-tuned, default) o `ThemeSource.custom` (tema activo de la librería). La pantalla ofrece un `SegmentedButton<ThemeModeOption>` de **3 segmentos** (System / Light / Dark) + un **switch master** *"Use a custom theme"* + un **colapsable** con las filas de la librería (2 built-ins: `Midnight` leans dark, `Sandstone` leans light; cada item expone Edit / Export / Delete vía `IconSurfaceMenu`; los built-ins están protegidos del Delete). El expansion/collapse del colapsable se persiste vía `customThemesExpandedProvider` + `AppearancePreferencesStore` (`uxnan.appearance.customThemesExpanded`). **Como hermanos del colapsable** (siempre visibles): **New theme** (dialog con seed HSV + Light/Dark → editor que abre en la pestaña elegida), **Import theme** (acepta un JSON object O un JSON array; ids duplicados se reasignan), **Export all themes** (sheet con *Copy to clipboard* o *Save to file* via `share_plus`) y **Reset library** (restaura built-ins + apaga el switch). Cada Export individual (per-row o editor) ofrece el mismo sheet clipboard/file. Cuando el switch está on, los 3 segmentos del picker quedan deshabilitados; cuando está off, el colapsable se renderiza greyed out + no interactivo (`IgnorePointer` + `Opacity`). El editor ([`CustomThemeEditorScreen`](../../uxnanmobile/lib/presentation/screens/settings/custom_theme_editor_screen.dart)) ofrece nombre + descripción, tabs Light/Dark, lista agrupada de roles M3 con HSV picker por rol (`ColorPickerSheet`), exportar a JSON (clipboard o archivo via `share_plus`) / importar desde JSON vía portapapeles, *"Derive from seed"* por modo; al guardar, si el id es **nuevo** auto-activa el tema + sincroniza `themeMode` con la pestaña activa (un tema nuevo en dark se aplica como `ThemeMode.dark`); editar uno existente solo hace upsert por id. Persistido en `shared_preferences` bajo `uxnan.appearance.customThemes` (JSON array), `uxnan.appearance.activeCustomThemeId`, `uxnan.appearance.useCustomTheme` y `uxnan.appearance.customThemesExpanded`. La clave legacy `uxnan.appearance.customTheme` se migra una sola vez al primer hydrate. Ver `02c-implementation-guide.md` §3.1 para la spec completa. |
