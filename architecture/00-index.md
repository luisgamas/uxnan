...
| **Verificador de actualizaciones in-app** (sin instalación silenciosa) | ✅ Hecho (code-complete; pendiente verificación on-device) | Chequeo throttled en launch/resume + *Buscar ahora* en Ajustes → Actualizaciones + banner *Actualización disponible* descartable. Android = Play In-App Update (flujo *immediate*); iOS = lookup de versión en App Store + abrir listing. Envuelve `flutter_upgrade_version` tras un `AppUpdateService` guardado. Android requiere un build del **track interno de Play** para reportar updates; iOS queda inerte hasta tener listing en App Store (`uxnanmobile/FOR-HUMAN.md`). El canal **APK/GitHub Releases** queda como follow-up (no lo cubre el plugin). |
