...
| **Native-session conversation convergence** (completed turns written from Codex/OpenCode/Claude/pi/Zero/Grok clients) | ✅ Done | Bridge reconciles every idle `turn/list`; Mobile polls the active idle conversation every 3 s and persists external user + assistant messages without duplicating its own turns. Antigravity is explicitly unsupported because `agy` exposes no readable transcript. **Both directions**: the bridge also releases the agent session between turns, so a conversation started on the phone opens in the agent's own client (Codex grants one writer per thread — 02a §5.8.8). |
| Conversación — **UI** (`ConversationScreen` con M3 + Neural Expressive, renderers, composer anclado abajo) cableada a datos reales | ✅ Hecho | Auto-follow cede al scroll manual; proceso secundario en disclosures tonales contraídos; prompts largos expandibles; composer reactivo al foco + franja plegable de contexto del turno; modelo/agente del thread, git por `cwd` real |
| **Indicador "Responding…" por thread** | ✅ Hecho | `ThreadActivity` + `threadActivityProvider`; antes del primer token la etiqueta usa skeleton, después desaparece y el loader avanza inline tras el último token como cursor de escritura |
| **Réplica del bridge: una sola capa** (`sync/changes` por revisión, registro de proyectos espejo, carpeta de inicio compartida, presencia del desktop, origen de cada conversación) | ✅ Hecho (pendiente verificación en dispositivo contra un desktop) | `BridgeReplica` es la única escritura de hilos y proyectos del bridge; cursor por PC en drift; notificaciones con `rev` (una tardía se descarta, un salto re-sincroniza). 02a §5.8.17 |
| Flujo **Nueva conversación** (proyecto del registro del PC — o **agregar uno** con `project/add` desde el **folder browser** `workspace/browseDirs` — + agente `agent/list` + modelo `agent/models`) + **selector de modelos** (`thread/setModel`, `AgentModel[]` estructurado) | ✅ Hecho | Diálogo M3 full-screen Neural Expressive; comparación directa de agentes en tarjetas de esquinas dinámicas (chips sólo en la selección); skip onboarding si ya hay PC |
| **Adjuntar imágenes** (image picker **multi-selección**, base64 inline, thumbnail strip, image-only message) | ✅ Hecho (gated by `AgentCapabilities.images`) | Envío en `turn/send { attachments }`; `text` ahora opcional. La tira de pendientes vive **dentro** del composer (56 dp, ✕ por imagen, scroll horizontal, tope de 10 por mensaje); ya enviada, la misma tira (72 dp) se dibuja **encima** de la burbuja y abre la imagen a tamaño completo al tocarla |
...
| **Context-compaction milestones** (durables, causa/tokens cuando existen) | ✅ Hecho | `CompactionContent` por `stream/content/block`; señales reales de Codex/Claude/OpenCode/pi, sin inferencia para Zero/Grok/Antigravity |
...
| **Interactive approval** (Approve / Reject / "allow session", persistido) | ✅ Hecho (app + bridge) | El bridge emite un `approval` content block; el móvil responde con `turn/send { approvalResponse }`. La decisión se persiste en SharedPreferences y la card queda en estado "Decision recorded" tras scroll/restart. Echo demo, Claude Code (`PreToolUse`), Codex (`app-server`), OpenCode (`permission.asked`) y Zero/Grok (ACP) están cableados; pi y Antigravity no exponen un canal pre-tool headless. |
...
| **Project-level thread scoping** | ✅ Hecho | La barra de filtros de threads abre con un **scope selector** (Agent / Project) a la izquierda; los chips a la derecha cambian según el scope activo (*All + cada agente* o *All + cada proyecto* por `projectId`/`cwd`). Solo un scope visible a la vez; cambiar de scope limpia el filtro de la otra dimensión. Sobre los hilos que mantiene la réplica (02a §5.8.17). (`threads_screen.dart` `_ScopeSelector`/`_ThreadScope`) |
| **Verificador de actualizaciones in-app** (sin instalación silenciosa) | ✅ Hecho (pendiente re-verificación on-device del flujo completo) | Chequeo en launch/resume con **intervalo configurable** (cada inicio / 6 / 12 / 24 por defecto / 48 h / semanal / mensual), **versión actual**, *Buscar ahora*, y un flujo **descargar → instalar dentro de Ajustes → Actualizaciones** (más el banner descartable en Conversaciones, en sync). Android = Play In-App Update **flexible** (descarga en background con % real + instalación in-app); iOS = lookup de versión en App Store (`dio`, iTunes) + overlay StoreKit `SKStoreProductViewController`. Un solo paquete `in_app_update_flutter` tras un `AppUpdateService` guardado. **Un update flexible es reanudable:** la descarga la posee Play y sobrevive al proceso que la inicia, así que el stream en vivo no puede ser la única fuente de verdad — cada chequeo relee la etapa que Play reporta (`AppUpdateStatus.installStage`, desde `AppUpdateInfo.installStatus`) y retoma el flujo donde está: un update ya iniciado sigue contando como disponible (Play pasa a reportar `developerTriggeredUpdateInProgress`, no `updateAvailable`), uno descargado vuelve como *Instalar ahora*, y un update pendiente **salta el intervalo** en cada foreground (el intervalo rige buscar una versión *nueva*, no terminar la que el usuario ya aceptó). Android requiere un build del **track interno de Play** para reportar updates; iOS queda inerte hasta tener listing en App Store (`uxnanmobile/FOR-HUMAN.md`). El canal **APK/GitHub Releases** queda como follow-up (no lo cubre el plugin). |
