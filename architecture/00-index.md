...
| **Verificador de actualizaciones in-app** (sin instalación silenciosa) | ✅ Hecho (code-complete; pendiente verificación on-device) | Chequeo en launch/resume con **intervalo configurable** (cada inicio / 6 / 12 / 24 por defecto / 48 h / semanal / mensual), **versión actual**, *Buscar ahora*, y un flujo **descargar → instalar dentro de Ajustes → Actualizaciones** (más el banner descartable en Conversaciones, en sync). Android = Play In-App Update **flexible** (descarga en background con % real + instalación in-app); iOS = lookup de versión en App Store (`dio`, iTunes) + overlay StoreKit `SKStoreProductViewController`. Un solo paquete `in_app_update_flutter` tras un `AppUpdateService` guardado. Android requiere un build del **track interno de Play** para reportar updates; iOS queda inerte hasta tener listing en App Store (`uxnanmobile/FOR-HUMAN.md`). El canal **APK/GitHub Releases** queda como follow-up (no lo cubre el plugin). |
