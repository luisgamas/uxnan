...
solo puede conducir **un turno por thread** — el agente one-shot reanuda su
sesion en cada turno (`claude -p --resume`), asi que un segundo turno
concurrente pondria dos procesos CLI sobre la misma sesion, y los agentes de
proceso residente (pi, Antigravity) leen un turno a la vez de stdin, asi que la
propia CLI encolaria el segundo como el turno siguiente. Por
