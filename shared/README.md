...
![Contracts](https://img.shields.io/badge/68_methods_%7C_10_notifications-blue?style=for-the-badge)
...
> **Status:** implemented and stable — **68 JSON-RPC methods** + **10 streaming
...
| Models | thread / turn / message (with `MessageContent` polymorphic blocks), durable `CompactionContentBlock`, git, workspace (incl. `browseDirs` + `exists`), project, auth, session/trust (`BridgeStatus` incl. `latestVersion?`/`updateAvailable?`), bridge-owned profile metrics (complete-ledger export/import), approval, question (interactive multiple-choice) |
