...
/// up, and the conversation actions still waiting to reach it (architecture/02a
/// §5.8.17). Conversations live in [IThreadRepository].
///
...
abstract class IBridgeReplicaRepository {
  ...
  /// Keeps [action] until it can be sent, superseding what it makes moot for
  /// the same conversation: a rename replaces an earlier rename, an archive or
  /// unarchive an earlier one of either, and a delete everything before it.
  Future<void> enqueueThreadAction(PendingThreadAction action);

  /// The actions waiting for [deviceId], in the order they were taken.
  Future<List<PendingThreadAction>> pendingThreadActions(String deviceId);

  /// Drops the kept action [id] (it was sent, or the bridge refused it).
  Future<void> removeThreadAction(int id);
